...
#define LINE_TOKEN  "qwrLPnEGjc0dHJXEhsYspSTP1dKBhawSC0J35qBBuMb" //สร้างตัวแปรสำหรับเก็บค่า Token
//
...
#include "SHTSensor.h"//เรียกใช้งานไฟล์ DHTSensor.h สำหรับการเรียกใช้งาน DHT Sensor
...
#define BLYNK_TEMPLATE_ID "TMPL6mcq-XhRv"
#define BLYNK_TEMPLATE_NAME "ESP8266"
#define BLYNK_AUTH_TOKEN "cwKttuzxkDI7e4xZo7lEIsdnP0UlbHOz"  //สร้างตัวแปรสำหรับเก็บค่า Token ของ Blynk
...
BLYNK_WRITE(V3) { // เมื่อมีการส่งค่าจาก V3
  if (autoMode == false) { // ทำการตรวจสอบตัวแปร autoMode ต้องไม่อยู่ในโหมด Manual
    digitalWrite(RELAY_1, param.asInt());  // HIGH =1 / LOW = 0 // สั่งงาน 1 Relay ทำงาน
  } else {
...
void setup() { // เมื่อบอร์ดเริ่มทำงาน
  ...
  initSHT(); //เปิดใช้งานเซ็นเซอร์อุนหภูมิ DHT
...
void runTime() {
  ...
  if (autoMode) { // เมื่อตัว autoMode มีค่าเท่ากับ 1 หรือ เมื่ออยู่ในโหมด Auto
    ...
    if (waterIsReady == false) {
      ...
      if (waterTicker == false) {
        ...
        oneTicker[5] = false;
        if (isConnected) {
          LINE.notify("ปริมาณน้ำในถังเต็มแล้ว !");
        }
        waterTicker = true;
      }
  ...
  Serial.printf("%d %d %d %d %d " , oneTicker[0],oneTicker[1],oneTicker[2],oneTicker[3],oneTicker[4]);
}
...
void loop() { // ลูปการทำงานหนัก
  ...
  SimulateTester();
  readSHT(); //อ่านจากเซ็นเซอร์จากค่า DHT
